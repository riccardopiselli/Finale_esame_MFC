...
mydf = pd.read_csv('Alaska.csv')


"""


Per l'Alaska sono presenti i valori per una sola stazione di monitoraggio i singoli inquinati
...
axs[0].set_title('andamenti temporali inquinnati', size = 18)
axs[0].legend()
axs[0].grid(True)
...
print('Alaska from 2013-12-01 to 2015-12-31:')
...
print('\n', df, '\n')
df.to_csv('Data_Alaska.csv',index = False)
...
maskCO = abs(freq_CO) < 0.035
maskO3 = abs(freq_O3) < 0.035
maskSO2 = abs(freq_SO2) < 0.035
maskNO2 = abs(freq_NO2) < 0.035
...
dfAlaska = pd.concat([xCO, xNO2, xSO2, xO3], axis=True)
dfAlaska.to_csv('_alaska.csv', index=False)
...
print('\n\nIl file Alaska.py è stato eseguito correttamente è sono stati creati due file csv:\n(1)Data_Alaska.py contenete delle informazione statistiche sul set di dati \n(2)_alaska.py che raccogli le frequenze di maggior importanza per la ricostrizione del segnale\n\n')