...
mydf = pd.read_csv('Minnesota.csv')


"""

Per l'Minnesota sono presenti i valori per una sola stazione di monitoraggio i singoli inquinati
...
print('Minnesota from 2013-12-01 to 2015-12-31:')
...
print('\n', df, '\n')
df.to_csv('Data_Minnesota.csv',index = False)
...
dfMinnesota = pd.concat([xCO, xNO2, xSO2, xO3], axis=True)
dfMinnesota.to_csv('_minnesota.csv', index=False)
...
print('\n\nIl file Minnesota.py è stato eseguito correttamente è sono stati creati due file csv:\n(1)Data_Minnesota.py contenete delle informazione statistiche sul set di dati \n(2)_minnesota.py che raccogli le frequenze di maggior importanza per la ricostrizione del segnale')