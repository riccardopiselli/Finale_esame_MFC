...
mydf = pd.read_csv('Alabama.csv')


"""


Per l'Alabama sono presenti i valori per una sola stazione di monitoraggio i singoli inquinati
...
axs[0].set_title('Andamenti temporali', size=20)
...
print('\n', df, '\n')
df.to_csv('Data_Alabama.csv',index = False)
...
dfAlabama = pd.concat([xCO, xNO2, xSO2, xO3], axis=True)
dfAlabama.to_csv('_alabama.csv', index=False)
...
print('\n\nIl file Alabama.py è stato eseguito correttamente è sono stati creati due file csv:\n(1)Data_Alabam.py contenete delle informazione statistiche sul set di dati \n(2)_alabama.py che raccogli le frequenze di maggior importanza per la ricostrizione del segnale')